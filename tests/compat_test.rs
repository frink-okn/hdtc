...
// ---------------------------------------------------------------------------
// Folder-based roundtrip tests
// Drop any RDF file into tests/data/roundtrip/ to add it to this test.
// ---------------------------------------------------------------------------

/// Map a file extension to the corresponding oxrdfio format.
/// Returns None for unrecognized extensions (those files are silently skipped).
fn detect_roundtrip_format(path: &Path) -> Option<oxrdfio::RdfFormat> {
    let ext = path
        .extension()
        .and_then(|e| e.to_str())
        .unwrap_or("")
        .to_lowercase();
    match ext.as_str() {
        "nt" | "ntriples" => Some(oxrdfio::RdfFormat::NTriples),
        "nq" | "nquads" => Some(oxrdfio::RdfFormat::NQuads),
        "ttl" | "turtle" => Some(oxrdfio::RdfFormat::Turtle),
        "trig" => Some(oxrdfio::RdfFormat::TriG),
        "rdf" | "xml" | "owl" => Some(oxrdfio::RdfFormat::RdfXml),
        "jsonld" | "json" => Some(oxrdfio::RdfFormat::JsonLd {
            profile: oxrdfio::JsonLdProfileSet::empty(),
        }),
        "n3" => Some(oxrdfio::RdfFormat::N3),
        _ => None,
    }
}

/// Parse an RDF file and return all triples as `[subject, predicate, object]` string arrays.
///
/// Terms use the HDT canonical form:
///   - URIs: bare string, e.g. `http://example.org/foo`
///   - Blank nodes: `_:label`
///   - Plain string literals: `"value"`
///   - Language-tagged literals: `"value"@lang`
///   - Typed literals: `"value"^^<datatype_uri>`
///
/// Quad formats (NQuads, TriG) have their graph component discarded.
/// Malformed triples are silently skipped, mirroring hdtc behaviour.
fn parse_rdf_to_triples(path: &Path, format: oxrdfio::RdfFormat) -> Result<Vec<[String; 3]>, String> {
    let file = std::fs::File::open(path).map_err(|e| e.to_string())?;
    let reader = std::io::BufReader::new(file);
    let parser = oxrdfio::RdfParser::from_format(format).lenient();
    ...
    for result in parser.for_reader(reader) {
        ...
        let s = match oxrdf::Term::from(quad.subject) {
            oxrdf::Term::BlankNode(b) => format!("_:{}", b.as_str()),
            oxrdf::Term::NamedNode(n) => n.as_str().to_string(),
            _ => unreachable!("subject cannot be a literal"),
        };
...
fn canonicalize_bnodes(triples: Vec<[String; 3]>) -> Vec<[String; 3]> {
    ...
    let mut result: Vec<[String; 3]> = triples
        .into_iter()
        .map(|t| {
            t.map(|term| {
...
/// Run the full roundtrip for a single RDF input file:
/// hdtc → HDT → hdt-java HDT2RDF → N-Triples → compare with original.
fn run_single_roundtrip(classpath: &str, input_path: &Path) -> Result<(), String> {
    let format = detect_roundtrip_format(input_path)
        .ok_or_else(|| format!("Unrecognized RDF extension: {}", input_path.display()))?;

    // Parse original RDF to get the expected triple set.
    let expected = parse_rdf_to_triples(input_path, format)
        .map_err(|e| format!("Failed to parse original RDF: {e}"))?;
    let expected_canonical = canonicalize_bnodes(expected);

    // Temp directory for this file's intermediate files.
    let temp = tempfile::tempdir().map_err(|e| e.to_string())?;

    // Convert to HDT.
    let hdt_path = run_hdtc_to_path(temp.path(), &[input_path], "test.hdt");

    // Convert HDT back to N-Triples using hdt-java HDT2RDF.
    let output_nt = temp.path().join("roundtrip.nt");
    let output = Command::new("java")
        .args([
            "-cp",
    ...
    // Parse the roundtrip output.
    let actual = parse_rdf_to_triples(&output_nt, oxrdfio::RdfFormat::NTriples)
        .map_err(|e| format!("Failed to parse HDT2RDF output: {e}"))?;
    let actual_canonical = canonicalize_bnodes(actual);

    // Compare.
    if actual_canonical.len() != expected_canonical.len() {
        return Err(format!(
            "Triple count mismatch: got {}, expected {}",
            actual_canonical.len(),
            expected_canonical.len()
        ));
    }
    for (i, (a, e)) in actual_canonical.iter().zip(expected_canonical.iter()).enumerate() {
        if a != e {
            return Err(format!(
                "Triple mismatch at position {i}:\n  actual:   {a:?}\n  expected: {e:?}"
            ));
        }
    }
...
/// Roundtrip compatibility test over all RDF files in `tests/data/roundtrip/`.
///
/// For each file the test:
///   1. Converts it to HDT using `hdtc`.
///   2. Converts the HDT back to N-Triples using hdt-java HDT2RDF.
///   3. Verifies the recovered triples are identical to the original.
///
/// To add a new test case just drop an RDF file into that directory — no code
/// changes needed.  Supported extensions: `.nt`, `.ttl`, `.rdf`, `.xml`,
/// `.jsonld`, `.n3`, `.nq`, `.trig`.
///
/// Requires Java and downloads hdt-java automatically on first run.
/// Run with: `cargo test -- --ignored test_hdtjava_roundtrip_rdf_files`
#[test]
#[ignore]
fn test_hdtjava_roundtrip_rdf_files() {
    let hdt_java_dir = ensure_hdt_java();
    let classpath = hdt_java_classpath(&hdt_java_dir);
    ...
    if !roundtrip_dir.exists() {
        eprintln!(
            "Roundtrip directory not found, skipping: {}",
            roundtrip_dir.display()
        );
        return;
    }

    let mut rdf_files: Vec<PathBuf> = std::fs::read_dir(&roundtrip_dir)
        .expect("read roundtrip directory")
        .filter_map(|e| e.ok())
        .map(|e| e.path())
        .filter(|p| p.is_file() && detect_roundtrip_format(p).is_some())
        .collect();
    rdf_files.sort(); // deterministic order across platforms

    if rdf_files.is_empty() {
        eprintln!(
            "No RDF files found in {}, skipping",
            roundtrip_dir.display()
        );
        return;
    }
    ...
    eprintln!(
        "Running roundtrip test on {} file(s) in {}",
        rdf_files.len(),
        roundtrip_dir.display()
    );
    ...
    for path in &rdf_files {
        ...
        eprint!("  {name} … ");
        match run_single_roundtrip(&classpath, path) {
            Ok(()) => eprintln!("PASS"),
            Err(e) => {
                eprintln!("FAIL");
