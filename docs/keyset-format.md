...
For the `subjects` and `objects` roles, the key set is the exact parent of both
sketches: a `.minhash` is its bottom `k` values, and a `.filter` is built from
it. A key set costs roughly **four times** a Fuse8 filter's bytes (§5), which is
the whole trade-off — a role below ~10⁸ keys is cheap enough to publish exactly,
and above that the sketches earn their approximation.
...
```text
keysets/
  subjects-only.keys  qualifying IRIs in the Subjects section
  objects-only.keys   qualifying IRIs in the Objects section
  shared.keys         qualifying IRIs in the Shared section
```
...
The default files are the HDT dictionary's own disjoint partition. The familiar
subject and object views are reconstructed by merging sorted files:
...
`subjects.keys` and `objects.keys` remain valid optional artifacts for consumers
that prefer the overlapping views directly. `predicates.keys` is also optional;
it cannot be reconstructed from the default trio because predicates have their
...
| `2` | predicates | qualifying terms in `Predicates` |
| `3` | shared | qualifying terms in `Shared` |
| `4` | subjects-only | qualifying terms in `Subjects` |
| `5` | objects-only | qualifying terms in `Objects` |
...
present in **both** composite roles. Roles `3`, `4`, and `5` encode the same
information without duplication: they are pairwise disjoint, and merging role
`3` with role `4` or `5` exactly reconstructs role `0` or `1`, respectively.

Role `2` contains predicate IRIs only. It has no sketch counterpart; the sketch
convention gives predicates no role ([sketch-format.md](sketch-format.md) §1.1)
because a dataset's predicate set is small and VoID property partitions describe
it exactly. A predicate IRI may also occur in another dictionary section; that
reuse matters when comparing roles but never duplicates an entry within
`predicates.keys`.
...
This applies to every role. A dictionary's literals and blank nodes never appear
in any key set, including section roles whose source section contains them.
...
Distinct *terms* are guaranteed by the source. The four dictionary sections are
internally distinct, and the two composite roles merge mutually disjoint
sections. Distinct *keys* are not guaranteed because two different IRIs can hash
to the same `u64`, so producers MUST still deduplicate. A duplicate key within
any role can only be a hash collision and is rare enough to be worth a warning.
...
| 13 | 1 | `u8` | `role` | `0` = subjects, `1` = objects, `2` = predicates, `3` = shared, `4` = subjects-only, `5` = objects-only |
...
5. `role` is one the reader implements. A reader that does not implement a role
   MUST reject it rather than infer its membership from the numeric id.
...
dataset for general consumption SHOULD emit all three disjoint roles (`shared`,
`subjects-only`, and `objects-only`), because a consumer that finds one role has
no way to tell "this role was not built" from "this dataset has no such IRIs" —
the empty file in rule 6 exists to make that distinction.
...
rejection rather than tolerance. A reader that implements only a subset of roles
rejects the others safely.
...
Each selected dictionary section is read once and fanned out to the composite or
section roles that draw from it.
